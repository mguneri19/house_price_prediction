...
df = pd.concat([train, test], ignore_index=True).reset_index()
...
def check_df(dataframe):
    ...
    print("##################### Quantiles #####################")
    # Sadece sayısal değişkenler için quantile hesapla
...
def high_correlated_cols(dataframe, plot=False, corr_th=0.70):
    # Sadece sayısal değişkenler için korelasyon hesapla
    ...
    if plot:
        ...
        plt.show()
    if drop_list:
        print(f"Yüksek korelasyonlu değişkenler: {drop_list}")
    return drop_list
...
if numeric_qual_cols:
    df["TotalQual"] = df[numeric_qual_cols].sum(axis=1)
else:
    df["TotalQual"] = 0
